...
; ##################################################################
; Fork of MikeOS -- The Mike Operating System kernel
; with small changes to adjust it to the arcadia os by @ophelia
...
; ##################################################################
	...
	%DEFINE ARCADIA_VER '1.0'	; OS version number
	%DEFINE ARCADIA_API_VER 16	; API version for programs to check
...
; use a arcadia system call without having to know its exact position
...
os_main:
	...
	mov ax, 0
	mov ss, ax					; setting stack segment and pointer
	mov sp, 0FFFFh
	sti							; restoring interrupts

	cld							; the default direction for string operations
								; will be 'up' - incrementing address in RAM

	mov ax, 2000h				; setting all segments to match to where the kernel is loaded
	mov ds, ax					; after this, we don't need to bother with segments again
	mov es, ax			
	mov fs, ax			
	mov gs, ax
	...
	cmp dl, 0
	je no_change
	mov [bootdev], dl			; saving boot device number
	push es
	mov ah, 8					; getting drive parameters
	int 13h
	pop es
...
no_change:
	mov ax, 1003h				; setting text output with certain attributes
...
no_autorun_bin:
	...
	call os_file_exists
	jc option_screen			; skipping next section if AUTORUN.BAS doesn't exist
...
option_screen:
	...
	mov cx, 00111111b			; Colour: white text on cyan
	...
	os_init_msg		db 'welcome to ARCADIA', 0
	os_version_msg		db 'version ', ARCADIA_VER, 0

	dialog_string_1		db 'have fun in the 16-bit paradise', 0
...
; valid -- in other words, that it has a .BIN extension
	...
	push si						; saving filename temporarily
	...
	dec si
	dec si
	dec si						; ...and to the start of extension!

	mov di, bin_ext
	mov cx, 3
	rep cmpsb					; are the final 3 chars 'BIN'?
...
execute_bin_program:
	...
	mov ax, 0					; clearing all registers
	mov bx, 0
	mov cx, 0
	mov dx, 0
	mov si, 0
...
not_bin_extension:
	pop si						; popping, because we pushed during the .BIN extension check

	push si						; saving it again in case of error
	...
	mov ax, si
	mov cx, 32768				; points to where to load the program file
	call os_load_file			; loading filename pointed to by AX
...
not_bas_extension:
	...
	jmp app_selector			; starting over again
	...
	kerndlg_string_1	db 'Cannot load and execute ARCADIA kernel!', 0
	kerndlg_string_2	db 'KERNEL.BIN is the core of ARCADIA, and', 0
